...
// Backup automático antes do seed
function createBackup(): string | null {
  try {
    ...
    console.error("⚠️  Erro ao criar backup:", error)
    return null
  }
}
...
async function main() {
  ...
  // Limpar dados existentes APENAS se SEED_FORCE_CLEAN=true
  // Por padrão, preserva dados existentes
  const shouldClean = process.env.SEED_FORCE_CLEAN === "true"
  
  if (shouldClean) {
    console.log("⚠️  SEED_FORCE_CLEAN=true - Limpando dados existentes...")
    ...
    console.log("ℹ️  Preservando dados existentes (use SEED_FORCE_CLEAN=true para limpar)")
  }
  ...
  await prisma.messageTemplate.createMany({
    data: [
      { name: "Boas-vindas", content: "Olá {{name}}! Obrigado pelo interesse na Qualivox. Como posso ajudá-lo(a)?", variables: JSON.stringify(["name"]), category: "boas-vindas" },
      ...
      { name: "Agendamento", content: "Olá {{name}}! Gostaria de agendar uma demonstração do Qualivox. Qual o melhor horário para você?", variables: JSON.stringify(["name"]), category: "agendamento" },
  ...
  console.log("   🔐 Login: admin@qualivox.com / admin123")
}
