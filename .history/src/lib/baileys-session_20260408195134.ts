...
async function _initSessionInternal(connectionId: string): Promise<Session> {
  ...
  wsocket.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === "open") {
      const phoneNumber = wsocket.user?.id?.split(":")[0]?.split("@")[0] ?? null
      await prisma.whatsappConnection.update({
        where: { id: connectionId },
        data: { status: "CONNECTED", qrcode: "", retries: 0, ...(phoneNumber ? { phoneNumber } : {}) },
      })
      sessions.set(connectionId, wsocket)
      connectedIds.add(connectionId)
      retriesQrCodeMap.delete(connectionId)

      console.log(`[Baileys] ${conn.name} CONNECTED phone=${phoneNumber}`)
      emitSSE("connection_update", { connectionId, status: "CONNECTED", phoneNumber })
    }
