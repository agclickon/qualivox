...
// Detecta se estamos usando PostgreSQL (produção/Supabase) ou SQLite (local)
const IS_POSTGRES = (process.env.DATABASE_URL || "").startsWith("postgresql")

const DATA_DIR = path.join(process.cwd(), "data")
const TENANTS_DIR = path.join(DATA_DIR, "tenants")

// Garante que os diretórios existem (apenas em ambiente SQLite local)
if (!IS_POSTGRES && !fs.existsSync(TENANTS_DIR)) {
...
export async function resolveTenant(userId: string): Promise<TenantContext | null> {
  // Em modo PostgreSQL (produção), tudo está no banco único do Supabase
  if (IS_POSTGRES) {
    return {
      companyId: "default",
      dbPath: "",
  ...
  const dbPath = isDefault
    ...
    : path.join(TENANTS_DIR, `qualivox-${companyId}.db`)
...
export async function getPrismaForTenant(companyId: string): Promise<PrismaClient> {
  // Em modo PostgreSQL (produção), retorna o Prisma padrão (Supabase)
  if (IS_POSTGRES) {
...
export async function provisionTenant(input: ProvisionTenantInput): Promise<string> {
  ...
  const dbPath = path.join(TENANTS_DIR, `qualivox-${companyId}.db`)
