...
import { getMediaFileBuffer } from "@/lib/media-storage"
...
async function getDefaultStageId(db = prisma): Promise<string | null> {
...
async function processAccumulatedMessage(
  ...
  const convRows = await db.$queryRaw<{ lead_id: string; assigned_to_id: string | null; status: string }[]>`
...
async function findOrCreateLead(phoneNumber: string, pushName?: string | null, connectionId?: string | null, db = prisma) {
  ...
  if (!lead) {
    lead = await (db as any).lead.create({
      data: {
        name: pushName || `WhatsApp ${phoneNumber}`,
        whatsappNumber: phoneNumber,
        phone: phoneNumber,
        source: "whatsapp",
        pipelineStageId: await getDefaultStageId(db),
    ...
    if (Object.keys(updates).length > 0) {
      lead = await (db as any).lead.update({ where: { id: lead.id }, data: updates })
    }
  }
...
async function updateLeadProfilePic(jid: string, imgUrl: string, db = prisma): Promise<void> {
...
async function upsertConversationFromChat(chat: any, connectionId: string, wbot?: any) {
  ...
  const db = await getPrismaForConnection(connectionId)
  const lead = await findOrCreateLead(phoneNumber, chat?.name || chat?.pushName, connectionId, db)
  ...
  await (db.conversation.upsert as any)({
...
async function extractMediaForAgent(
  ...
  try {
    const buffer = await getMediaFileBuffer(mediaUrl)
...
async function sendSupervisorAlert(
  ...
  try {
    ...
    const db = await getPrismaForConnection(connectionId)
    const allAgents = await db.$queryRaw<{ supervisor_phone: string | null; name: string; connection_ids: string }[]>`
      SELECT name, supervisor_phone, connection_ids FROM agents
      WHERE is_active = true AND supervisor_phone IS NOT NULL AND supervisor_phone != ''
    `
...
async function analyzeSentimentAndPersist(
  ...
  console.log(`[Sentiment] analyzeSentimentAndPersist chamado: leadId=${leadId}, conversationId=${conversationId}`)
  const db = await getPrismaForConnection(connectionId)
  ...
  console.log(`[Sentiment] Encontradas ${recentMessages.length} mensagens para análise`)
  ...
  if (!texts.length) {
    console.log(`[Sentiment] Nenhum texto para analisar`)
    return
  }

  console.log(`[Sentiment] Chamando analyzeSentiment com ${texts.length} textos`)
  const result = await analyzeSentiment(texts, leadName)
  if (!result) {
    console.log(`[Sentiment] analyzeSentiment retornou null`)
    return
  }
  console.log(`[Sentiment] Resultado: score=${result.score}, label=${result.label}, urgency=${result.urgency}`)
  ...
  // Atualiza o lead com score de qualificação baseado no sentimento (0-100)
  // Converte score de sentimento (-100 a 100) para score de qualificação (0 a 100)
  const qualificationScore = Math.max(0, Math.min(100, Math.round((score + 100) / 2)))
  const qualificationLevel = qualificationScore >= 70 ? "quente" : qualificationScore >= 40 ? "morno" : qualificationScore >= 20 ? "frio" : "nao_qualificado"

  console.log(`[Sentiment] Atualizando lead ${leadId}: score=${qualificationScore}, level=${qualificationLevel}`)
  await db.lead.update({
    where: { id: leadId },
    data: {
  ...
  console.log(`[Sentiment] Lead ${leadId} atualizado com sucesso`)
...
async function handleMessage(
  ...
  try {
    // Resolve o banco do tenant correto para esta conexão
    const db = await getPrismaForConnection(connectionId)

    // Buscar ou criar lead primeiro (para poder buscar foto de perfil)
    const lead = await findOrCreateLead(phoneNumber, pushName, connectionId, db)
    ...
    const existing = await db.$queryRawUnsafe<Array<{ external_id: string | null }>>(
      `SELECT external_id FROM messages WHERE external_id = ?`,
      msgId
    )
    if (existing.length > 0) {
      if (fromMe) {
        const persisted = await db.message.findFirst({
    ...
    if (!conversation) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const connRecord = connectionId ? await (db.whatsappConnection.findUnique as any)({
      ...
      if (assignToId) {
        const limitSetting = await db.setting.findUnique({ where: { key: "maxConvSimultaneous" } })
    ...
    if (!fromMe) {
      console.log(`[Sentiment] Iniciando análise para lead ${lead.id} (${lead.name}), conversation ${conversation.id}`)
      analyzeSentimentAndPersist(conversation.id, lead.id, messageContent, lead.name, sseId, connectionId)
        .then(() => console.log(`[Sentiment] Análise concluída para lead ${lead.id}`))
    ...
    if (!fromMe) {
      ...
      if (mediaResult?.mediaUrl && ["audio", "image", "document"].includes(mediaResult.messageType)) {
        ...
        if (extracted) {
          ...
          try {
            const row = await db.message.findUnique({ where: { id: sseId }, select: { metadata: true } })
            if (row) {
...
async function handleMsgAck(msg: WAMessageUpdate, connectionId: string): Promise<void> {
  try {
...
export function startMessageListener(
  ...
  ev.on("contacts.set", async (payload: unknown) => {
    ...
    const contactsDb = await getPrismaForConnection(connectionId)
    for (const contact of contacts) {
      if (contact.imgUrl) {
        await updateLeadProfilePic(contact.id, contact.imgUrl, contactsDb)
      }
    }
  })
  ...
  ev.on("contacts.update", async (updates: any[]) => {
    ...
    for (const update of updates || []) {
      if (update.imgUrl) {
        await updateLeadProfilePic(update.id, update.imgUrl, contactsDb)
      }
    }
  })
  ...
  ev.on("messages.update", (messageUpdate: WAMessageUpdate[]) => {
    ...
    for (const update of messageUpdate) {
      handleMsgAck(update, connectionId)
    }
  })
