...
export async function analyzeSentiment(
  ...
  const prompt = `Analise o sentimento do lead "${leadName}" com base nas últimas mensagens dele:\n\n${msgs}\n\nRetorne APENAS JSON válido (sem markdown):\n{"score": 0, "label": "neutro", "urgency": 0, "flags": []}\n\nRegras de pontuação (USE TODA A ESCALA):\n- score: -100 a 100. Positivo claro = 50 a 90. Negativo claro = -50 a -90. Neutro = -20 a 20.\n- label: "positivo" se score > 20 | "negativo" se score < -20 | "neutro" caso contrário\n- urgency: 0-100 (alta urgência = lead com pressa ou frustrado)\n- flags: ["frustrado","impaciente","satisfeito","ameaca_cancelamento","elogio","interesse_alto"]`
