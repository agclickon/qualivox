...
async function callOpenAI(messages: AiMessage[], apiKey: string, model: string, temperature: number): Promise<string> {
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
    body: JSON.stringify({ model, messages, temperature, max_tokens: 2000, response_format: { type: "json_object" } }),
  })
...
async function callGemini(messages: AiMessage[], apiKey: string, model: string, temperature: number): Promise<string> {
  ...
  const res = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`,
    {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { temperature, maxOutputTokens: 2000, responseMimeType: "application/json" },
      }),
    }
  )
...
export async function aiChat(messages: AiMessage[], temperature = 0.3): Promise<string> {
  ...
  switch (provider) {
    ...
    case "gemini":    return callGemini(messages, apiKey, model, temperature)
    ...
    default:          return callOpenAI(messages, apiKey, model, temperature)
  }
}
