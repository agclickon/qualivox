  ...
  try {
    ...
    if (!conversationId) {
      ...
      const lead = await prisma.lead.findFirst({
        where: { OR: [{ whatsappNumber: { contains: phoneNumber } }, { phone: { contains: phoneNumber } }] },
      })
      if (lead) {
        let conversation = await prisma.conversation.findFirst({ where: { leadId: lead.id, connectionId } })
        if (!conversation) {
          conversation = await prisma.conversation.create({ data: { leadId: lead.id, whatsappChatId: jid, connectionId } })
        }
