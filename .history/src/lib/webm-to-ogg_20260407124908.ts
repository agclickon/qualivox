...
async function buildWaveformFromPcm(ogg: Buffer): Promise<Uint8Array> {
  ...
  try {
    ...
    // Compressão dinâmica logarítmica para waveform mais natural (similar ao WhatsApp)
    // Usa curva logarítmica: valores baixos ficam proporcionalmente mais baixos,
    // valores altos mantêm a relativa altura, evitando a normalização linear
    const max = Math.max(...vals, 1e-10)
    const LOG_SCALE = 2.5 // Fator de compressão - ajustável
    return new Uint8Array(vals.map(v => {
      // Normaliza para 0-1, aplica curva logarítmica, converte para 0-100
      const normalized = v / max
      const compressed = Math.log1p(normalized * LOG_SCALE) / Math.log1p(LOG_SCALE)
      return Math.min(100, Math.max(0, Math.floor(compressed * 100)))
    }))
  } catch (err) {
