...
async function findActiveAgent(connectionId: string): Promise<any | null> {
  // Usa o banco do tenant correto para esta conexão
  const db = await getPrismaForConnection(connectionId)
  const agents = await (db.agent as any).findMany({
    where: { isActive: true },
  })
...
async function getLeadContext(leadId: string, db = prisma): Promise<LeadContext | null> {
...
function buildSystemPrompt(
  ...
  calendarEnabledOverride?: boolean
): string {
  ...
  const calendarEnabled = calendarEnabledOverride ?? Boolean((agent as any).calendar_enabled ?? (agent as any).calendarEnabled)
...
function parseAgentResponse(raw: string): {
  ...
  try {
    ...
    // Se ainda não começa com {, tenta encontrar o primeiro { ... } no texto
...
async function getLastSentimentScore(conversationId: string, db = prisma): Promise<number | null> {
  try {
    const rows = await db.$queryRaw<Array<{ sentiment_score: number | null }>>`
...
export async function processMessageWithAgent(
  ...
  try {
    ...
    // Resolve banco do tenant uma vez para todas as queries
    const db = await getPrismaForConnection(connectionId)

    // 2. Verifica se a conversa já tem atendente humano ativo (não processa nesses casos)
    const convRows = await db.$queryRaw<any[]>`SELECT assigned_to_id, status, agent_paused_until FROM conversations WHERE id = ${conversationId}`
    ...
    const [lead, history, ragContext] = await Promise.all([
      getLeadContext(leadId, db),
      getConversationHistory(conversationId, 20, db),
      searchKnowledge(agent.id, messageContent),
    ])

    if (!lead) return noOp

    // 3.5. Lê configurações de calendário via raw SQL (calendar_enabled foi adicionado
    //      por lazy migration e não existe no modelo Prisma gerado)
    let agentCalendarEnabled = false
    let agentCalendarAddMeetLink = true
    let agentOwnerId: string | null = null
    let busySlots: { start: string; end: string }[] = []
    let hasAgentEvent = false

    try {
      const agentRaw = await db.$queryRaw<{
        created_by_id: string | null
        calendar_enabled: number | null
        calendar_add_meet_link: number | null
      }[]>`
        SELECT created_by_id, calendar_enabled, calendar_add_meet_link FROM agents WHERE id = ${agent.id}
      `
      const ar = agentRaw[0]
      agentCalendarEnabled = Boolean(ar?.calendar_enabled)
      agentCalendarAddMeetLink = ar?.calendar_add_meet_link !== null ? Boolean(ar?.calendar_add_meet_link) : true
      agentOwnerId = ar?.created_by_id ?? null

      if (agentCalendarEnabled && agentOwnerId) {
        const timeMin = new Date().toISOString()
        const timeMax = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()
        const [slots, existingEvent] = await Promise.all([
          getBusySlotsForRange(agentOwnerId, timeMin, timeMax),
      ...
      console.warn("[Orchestrator] Falha ao ler dados de calendário do agente:", err)
    }

    console.log(`[Orchestrator] calendarEnabled=${agentCalendarEnabled} ownerId=${agentOwnerId} busySlots=${busySlots.length} hasAgentEvent=${hasAgentEvent}`)
    ...
    const systemPrompt = buildSystemPrompt(agent, lead, ragContext, busySlots, hasAgentEvent, agentCalendarEnabled)
    ...
    console.log(`[Orchestrator] calendarEnabled=${agentCalendarEnabled} | busySlots=${busySlots.length} | hasAgentEvent=${hasAgentEvent}`)
    const calIdx = systemPrompt.indexOf("REGRA CRÍTICA")
    if (calIdx >= 0) console.log(`[Orchestrator] CALENDAR_INSTRUCTION: ${systemPrompt.slice(calIdx, calIdx + 200)}`)
    else console.log(`[Orchestrator] CALENDAR_INSTRUCTION: NÃO ENCONTRADO NO PROMPT`)

    const rawResponse = await aiChat(messages, agent.temperature ?? 0.3, true)
    console.log(`[Orchestrator] RAW_RESPONSE: ${rawResponse.slice(0, 2000)}`)
    ...
    console.log(`[Orchestrator] calendarEvent=${JSON.stringify(calendarEvent)} | calendarEnabled=${agentCalendarEnabled}`)
    ...
    if (!escalate && agent.escalateThreshold) {
      const lastSentiment = await getLastSentimentScore(conversationId, db)
    ...
    // 7. Escalada
    if (shouldEscalate) {
      await escalateConversation(conversationId, finalEscalationReason || "Solicitado pelo agente", agent.name, leadId, db)
      return {
        handled: true,
        agentId: agent.id,
        reply,
    ...
    if (agentCalendarEnabled && agentOwnerId && (calendarEvent || cancelEvent || rescheduleEvent)) {
      try {
          ...
          if (integration?.is_active) {
            ...
            if (calendarEvent) {
              ...
              if (conflict) {
                ...
                const addMeetLink = calendarEvent.addMeetLink ?? agentCalendarAddMeetLink
    ...
    if ((agent.mode === "autonomous" || agent.mode === "hybrid") && jid) {
      try {
        ...
        const agentRows = await db.$queryRaw<any[]>`SELECT typing_delay, typing_delay_max, mark_as_read, split_messages, voice_enabled, voice_mode, voice_id, voice_speed, voice_stability, voice_similarity FROM agents WHERE id = ${agent.id}`
