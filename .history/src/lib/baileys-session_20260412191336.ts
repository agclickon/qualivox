...
async function loadAuthState(connectionId: string, db?: typeof prisma): Promise<{
  ...
  const p = db ?? prisma
  const isDefault = p === prisma
  const conn = await p.whatsappConnection.findUnique({
    where: { id: connectionId },
  })
  console.log(`[Baileys] loadAuthState ${connectionId} banco=${isDefault ? "DEFAULT" : "TENANT"} conn=${conn ? "found" : "NOT FOUND"} session=${conn?.session?.length ?? 0}b`)
