...
import { listPendingReminders, markReminderSent, markReminderFailed, getReminderSettings } from "@/lib/reminder-service"
      ...
      for (const { db } of tenants) {
        ...
        for (const reminder of pending) {
          try {
            ...
            const leadRows = await db.$queryRaw<{ name: string }[]>`
            ...
            if (reminder.google_event_id) {
              const evtRows = await db.$queryRawUnsafe<{ title: string; start_time: string }[]>(
            ...
            if (result.success) {
              await markReminderSent(reminder.id, db)
              console.log(
                `[ReminderRunner] ✓ Lembrete "${reminder.reminder_type}" enviado para lead ${reminder.lead_id}`
              )
            } else {
              await markReminderFailed(reminder.id, result.error, db)
            }
          } catch (err) {
            const msg = err instanceof Error ? err.message : "Erro desconhecido"
            await markReminderFailed(reminder.id, msg, db)
          }
        }
      }
    } catch (err) {
