    ...
    if ((agent.mode === "autonomous" || agent.mode === "hybrid") && jid) {
      try {
        ...
        if (useVoice && voiceId && sendAudioFn) {
          // Voz: verifica key ANTES de mostrar indicador de gravação
          ...
          if (elevenLabsKey) {
            try {
              ...
              console.error("[Orchestrator] Erro na síntese de voz, usando texto:", voiceErr)
              await presenceUpdate?.("paused", jid).catch(() => {})
              // Fallback para texto dividido
              ...
              for (const part of parts) {
                ...
                if (sendFn) await sendFn(connectionId, jid, part)
              }
              sentAutomatically = true
            }
          } else {
            // Sem API key: avisa no log e envia como texto normalmente
          ...
          sentAutomatically = true
          console.log(`[Orchestrator] ✓ ${parts.length} parte(s) enviada(s) para ${lead.name}`)
        }
      } catch (sendErr) {
