...
async function ensureDeliveriesTable(db: PrismaClient = prisma) {
  ...
  await db.$executeRawUnsafe(`ALTER TABLE webhooks ADD COLUMN name TEXT NOT NULL DEFAULT ''`).catch(() => {})
}
 ...
 */
export async function fireWebhook(event: string, data: Record<string, unknown>, db: PrismaClient = prisma): Promise<void> {
  await ensureDeliveriesTable(db)

  const endpoints = await db.$queryRawUnsafe<{
  ...
  await Promise.allSettled(
    eligible.map((ep) => deliverToEndpoint(ep.id, ep.url, ep.secret, event, body, 1, db))
  )
}
...
async function deliverToEndpoint(
  ...
  try {
    const headers: Record<string, string> = {
      ...
      "X-Qualivox-Event": event,
      "X-Qualivox-Delivery": deliveryId,
    }
    if (secret) {
      headers["X-Qualivox-Signature"] = generateSignature(secret, body)
    }
    ...
    if (success) {
      ...
      await scheduleRetry(deliveryId, webhookId, url, secret, event, body, attempt, statusCode, responseBody, db)
    }
  } catch (err) {
    const errMsg = err instanceof Error ? err.message : String(err)
    await scheduleRetry(deliveryId, webhookId, url, secret, event, body, attempt, 0, errMsg, db)
  }
}
...
async function scheduleRetry(
  ...
  const delayMs = RETRY_DELAYS_MS[attempt] ?? null
  ...
  if (delayMs !== null && nextAttempt <= RETRY_DELAYS_MS.length) {
    ...
    setTimeout(() => {
      deliverToEndpoint(webhookId, url, secret, event, body, nextAttempt, db).catch(() => {})
    }, delayMs)
  } else {
    await db.$executeRawUnsafe(
 ...
 */
export async function retryDelivery(deliveryId: string, db: PrismaClient = prisma): Promise<void> {
  await ensureDeliveriesTable(db)
  ...
  await deliverToEndpoint(d.webhook_id, d.url, d.secret, d.event, d.payload, d.attempt + 1, db)
}

/**
 * Monta payload enriquecido de conversão de lead.
 */
export async function buildLeadConvertedPayload(leadId: string, source: "manual" | "automatic", db: PrismaClient = prisma): Promise<Record<string, unknown>> {
  ...
  const interactions = await db.$queryRawUnsafe<{ type: string; content: string; created_at: string }[]>(
    `SELECT type, content, created_at FROM interactions WHERE lead_id = ? ORDER BY created_at DESC LIMIT 50`,
    leadId
  )

  const aiRow = await db.$queryRawUnsafe<{ sentiment_score: number | null; classification: string | null; extracted_data: string | null; created_at: string }[]>(
  ...
  const tags = await db.$queryRawUnsafe<{ name: string; color_hex: string }[]>(
