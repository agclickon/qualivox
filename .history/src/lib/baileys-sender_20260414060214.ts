...
export async function sendMediaMessage(opts: SendMediaOptions): Promise<{ success: boolean; messageId?: string; error?: string }> {
  ...
  try {
    ...
    if (!conversationId) {
      ...
      if (jid.endsWith("@lid")) {
        ...
        if (lead) {
          ...
          if (!conversation) {
            conversation = await (db.conversation.create as any)({ 
    ...
    const relativePath = await saveMedia({ conversationId, extension: ext, buffer, prefix: mediaType })
    console.log(`[sendMediaMessage] Arquivo salvo no Supabase: ${relativePath} (${buffer.byteLength} bytes)`)

    // Build Baileys message content using Buffer directly
    let content: Record<string, unknown>
    if (mediaType === "image" || mediaType === "sticker") {
      content = mediaType === "sticker"
        ? { sticker: buffer, mimetype: mimeType }
    ...
    console.log(`[sendMediaMessage] session.user=${session.user.id} — enviando via buffer`)
    ...
    const dbForInsert = await getPrismaForConnection(connectionId)
    const newId = crypto.randomUUID()
    await dbForInsert.$executeRawUnsafe(
    ...
    await dbForInsert.conversation.update({ where: { id: conversationId }, data: { lastMessageAt: timestamp } })
...
export async function sendTextMessage(
  ...
  try {
    ...
    if (lead) {
      ...
      if (!conversation) return { success: true, messageId: msgId }
