...
async function handleMessage(
  ...
  try {
    ...
    emitNewMessage({
      ...
      isRead,
      messageType: mediaResult?.messageType || "text",
    ...
    if (!fromMe) {
      ...
      if (agentContent) {
        // Passa funções de sessão diretamente do wbot para garantir acesso correto
        ...
        processMessageWithAgent(
          ...
          async (connId, jid, buffer, mimeType, convId) => {
            ...
            await sendMediaMessage({ 
              ...
              fileName: "voice.mp3", 
              ptt: true,
              conversationId: convId || conversation.id
            })
          },
          { id: msg.key.id!, remoteJid: remoteJid, fromMe: false },
          sessionFns,
          incomingMediaType
        ).catch((err) => console.error("[Orchestrator] Erro no processamento:", err))
      }
    }
  } catch (err) {
