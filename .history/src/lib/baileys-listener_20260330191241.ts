...
async function fetchAndSaveProfilePic(
  ...
  try {
    const url = await Promise.race([
      wbot.profilePictureUrl(jid, "preview").catch((err: any) => {
        console.log(`[Baileys] profilePictureUrl error for ${jid}:`, err?.message || err)
        return null
      }),
      new Promise<null>((resolve) => setTimeout(() => resolve(null), 5000)),
    ])

    console.log(`[Baileys] Profile pic URL for ${jid}:`, url ? url.substring(0, 50) + '...' : 'NULL')
    ...
    if (url) {
      ...
      console.log(`[Baileys] No profile pic available for ${jid}`)
    }
  } catch (err: any) {
...
async function handleMessage(
  ...
  try {
    ...
    // Buscar foto de perfil em background (não bloqueia)
    if (wbot && !fromMe && !lead.profilePicUrl) {
      fetchAndSaveProfilePic(wbot, remoteJid, lead.id).catch(() => {})
    }
