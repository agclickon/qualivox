...
function makeOpusTags(): Buffer {
  const vendor = Buffer.from('Qualivox', 'utf8')
...
async function buildWaveformFromPcm(ogg: Buffer): Promise<Uint8Array> {
  ...
  try {
    ...
    // A curva y = x^0.7 (mais linear que raiz quadrada) mantém dinâmica natural
    // Offset mínimo garante que valores baixos ainda sejam visíveis (15% mínimo)
    const max = Math.max(...vals, 1e-10)
    const POWER = 0.7 // Expoente da curva - mais próximo de 1 = mais linear
    const MIN_VALUE = 15 // Valor mínimo em escala 0-100 (15% de altura)
    const result = new Uint8Array(vals.map(v => {
      const normalized = v / max
      // Curva de potência com offset mínimo
      const curved = Math.pow(normalized, POWER)
      const withOffset = MIN_VALUE + (curved * (100 - MIN_VALUE))
      return Math.min(100, Math.max(0, Math.floor(withOffset)))
    }))
