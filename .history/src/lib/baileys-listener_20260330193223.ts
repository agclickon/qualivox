...
import { getContactFromStore } from "@/lib/baileys-session"
...
async function fetchAndSaveProfilePic(
  ...
  try {
    ...
    // 1. Tentar buscar do store do Baileys (cache local)
    if (connectionId) {
      const contact = getContactFromStore(connectionId, jid)
      if (contact?.imgUrl) {
        url = contact.imgUrl
      }
    }

    // 2. Se não encontrou no store, tentar via API do WhatsApp
    // Nota: @lid pode não funcionar, mas tentamos mesmo assim
    if (!url && wbot?.profilePictureUrl) {
      url = await Promise.race([
        wbot.profilePictureUrl(jid, "preview").catch(() => null),
    ...
    // Silenciar erros - foto de perfil é opcional
  }
}
...
async function handleMessage(
  ...
  // Log adicional para debug de foto de perfil
  if (msg.verifiedBizName) {
    console.log(`[Baileys] verifiedBizName: ${msg.verifiedBizName}`)
  }
