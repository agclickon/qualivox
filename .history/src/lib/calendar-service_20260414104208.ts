...
export async function getGoogleAuthUrl(userId: string): Promise<string> {
  ...
  console.log("[Google Calendar] Gerando URL de auth:", {
    redirectUri: creds.redirectUri,
    clientId: creds.clientId.substring(0, 15) + "..."
  })
...
export async function saveGoogleTokens(
  ...
  // Usa upsert via Prisma Client API (compatível com PostgreSQL e SQLite)
  const result = await prisma.calendarIntegration.upsert({
    where: {
      userId_provider: {
        userId: userId,
        provider: 'google'
    ...
    update: {
      ...
      refreshToken: tokens.refresh_token ? { set: tokens.refresh_token } : undefined,
...
export async function getAuthedClient(userId: string) {
  const rows = await prisma.$queryRaw<{
    ...
    WHERE user_id = ${userId} AND provider = 'google' AND is_active = true
  `
