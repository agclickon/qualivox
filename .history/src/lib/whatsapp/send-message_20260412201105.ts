...
import { emitNewMessage } from "@/lib/baileys-listener"
...
export async function sendMessageToLead(options: SendMessageOptions): Promise<SendMessageResult> {
  const { leadId, message, templateId, variables, quoted, conversationId, connectionId: optConnectionId, db: passedDb } = options
  // Se nenhum db foi passado mas temos connectionId, resolve o banco do tenant
  let db: typeof prisma = passedDb ?? prisma
  ...
  if (templateId) {
    const template = await db.messageTemplate.findFirst({ where: { id: templateId, isActive: true } })
    if (!template) {
  ...
  const defaultConnection = await db.whatsappConnection.findFirst({ where: { isDefault: true } })
  ...
  let connectionId = optConnectionId || targetConversation?.connectionId || defaultConnection?.id

  // Resolve db do tenant se ainda não foi resolvido e temos connectionId
  if (!passedDb && connectionId) {
    db = await getPrismaForConnection(connectionId)
  }

  let jid = targetConversation?.whatsappChatId || formatJid(phone)

  if (!targetConversation && connectionId) {
    targetConversation = await db.conversation.findFirst({ where: { leadId, connectionId } })
  ...
  await db.lead.update({ where: { id: leadId }, data: { lastInteraction: new Date() } })

  let persistedMessage: Awaited<ReturnType<typeof db.message.create>> | null = null
  if (targetConversation) {
    persistedMessage = await db.message.create({
      data: {
        ...
        metadata: quoted
          ? JSON.stringify({
              ...
              quotedFromMe: quoted.fromMe ?? false,
              quotedContent: quoted.content ?? "",
              quotedMessageType: quoted.messageType ?? "text",
            })
          : null,
      },
    })
    ...
    emitNewMessage({
      ...
      content: finalMessage,
      direction: "outgoing",
      createdAt: persistedMessage.createdAt instanceof Date
        ? persistedMessage.createdAt.toISOString()
        : new Date(persistedMessage.createdAt).toISOString(),
      conversationId: targetConversation.id,
      isRead: true,
      messageType: "text",
      mediaUrl: null,
