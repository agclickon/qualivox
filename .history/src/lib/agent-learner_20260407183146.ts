...
export async function indexConversation(
  ...
  try {
    ...
    let chunkCount = 0
    if (autoApprove) {
      chunkCount = await chunkAndEmbed(agentId, knowledgeId, content)
...
export async function triggerLearningForConversation(
  ...
  try {
    // Busca connectionId da conversa
    const convRows = await prisma.$queryRaw<Array<{ connection_id: string | null }>>`
    ...
    for (const agent of agents) {
      ...
      indexConversation(agent.id, conversationId, autoApprove).catch((err) => {
