...
import { GripVertical, User, X, Eye, Edit, Phone, Mail, Building2, Star, Flame, Thermometer, Snowflake, Loader2, Clock, MessageSquare, CheckCircle2, AlertCircle, Save } from "lucide-react"
...
type TimelineItem = {
  ...
  createdAt: string; user: { name: string } | null; connectionName: string | null
}

function LeadDetailModal({ lead: initialLead, onClose, onUpdated }: {
  lead: KanbanLead
  onClose: () => void
  onUpdated: (id: string, data: Partial<KanbanLead>) => void
}) {
  ...
  const [qualificationLevel, setQualificationLevel] = useState(lead.qualificationLevel || "nao_qualificado")
  ...
  const loadTimeline = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) setTimelineItems(data.data.items)
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/leads/${lead.id}`, {
        ...
        body: JSON.stringify({ name, email: email || null, phone: phone || null, companyName: companyName || null, status, source, qualificationLevel, score: Number(score) }),
      })
      const data = await res.json()
      if (data.success) {
        const updated = { ...lead, name, email: email || null, phone: phone || null, companyName: companyName || null, status, source, qualificationLevel, score: Number(score) }
        setLead(updated)
        onUpdated(lead.id, updated)
        setEditing(false)
  ...
  const TIMELINE_ICONS: Record<string, { icon: React.ElementType; color: string }> = {
    ...
    message_received: { icon: MessageSquare, color: "text-green-400" },
    stage_changed:    { icon: CheckCircle2,  color: "text-primary" },
    qualification:    { icon: Star,          color: "text-amber-400" },
    follow_up_sent:   { icon: Clock,         color: "text-purple-400" },
    escalation:       { icon: AlertCircle,   color: "text-red-400" },
    note:             { icon: Edit,          color: "text-muted-foreground" },
  }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="w-full max-w-lg rounded-xl bg-card border border-border shadow-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        ...
        <div className="flex gap-1 px-6 pt-4 flex-shrink-0">
          <button
            ...
            className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${tab === "info" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground hover:bg-accent"}`}
          >Informações</button>
          <button
            onClick={() => setTab("timeline")}
            className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${tab === "timeline" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground hover:bg-accent"}`}
          >Timeline</button>
        </div>
              ...
              {lead.assignedTo && (
                <div className="rounded-lg border bg-muted/20 p-3">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <div className="h-6 w-6 rounded-full bg-secondary flex items-center justify-center text-[9px] font-bold">{getInitials(lead.assignedTo.name)}</div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5 col-span-2">
                  ...
                  <input className="flex h-10 w-full rounded-lg border border-input bg-muted px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" value={name} onChange={(e) => setName(e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">Empresa</label>
                  <input className="flex h-10 w-full rounded-lg border border-input bg-muted px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" value={companyName} onChange={(e) => setCompanyName(e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">E-mail</label>
                  <input type="email" className="flex h-10 w-full rounded-lg border border-input bg-muted px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">Telefone/WhatsApp</label>
                  <input className="flex h-10 w-full rounded-lg border border-input bg-muted px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" value={phone} onChange={(e) => setPhone(e.target.value)} />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">Nível de qualificação</label>
                  <select className={selectCls} value={qualificationLevel} onChange={(e) => setQualificationLevel(e.target.value)}>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="number" min="0" max="100" className="flex h-10 w-full rounded-lg border border-input bg-muted px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" value={score} onChange={(e) => setScore(e.target.value)} />
                </div>
              </div>
            </div>
          )}
              ...
              {timelineLoading ? (
                ...
                <div className="relative pl-5 border-l border-border space-y-4">
                  {timelineItems.map((item) => {
                    const iconDef = TIMELINE_ICONS[item.type] ?? { icon: Clock, color: "text-muted-foreground" }
                    const Icon = iconDef.icon
                    return (
                      <div key={item.id} className="relative">
                        <div className={`absolute -left-[22px] flex h-4 w-4 items-center justify-center rounded-full bg-card border border-border ${iconDef.color}`}>
                        ...
                        <div className="rounded-lg border bg-muted/20 p-3">
                          ...
                          {(item.user || item.connectionName) && (
                            <div className="flex gap-2 mt-1.5">
                              {item.user && <span className="text-[10px] text-muted-foreground">por {item.user.name}</span>}
                              {item.connectionName && <span className="text-[10px] text-muted-foreground">· {item.connectionName}</span>}
                            </div>
                          )}
                        </div>
                      </div>
      ...
      {detailLead && (
        <LeadDetailModal
          ...
          onUpdated={(id, data) => {
            ...
            setDetailLead((prev) => prev ? { ...prev, ...data } : prev)
          }}
        />
      )}
    </div>
