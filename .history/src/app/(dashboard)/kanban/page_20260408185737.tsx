...
import { GripVertical, User, X, Eye, Edit, Phone, Mail, Building2, Star, Flame, Thermometer, Snowflake, Loader2 } from "lucide-react"
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="w-full max-w-lg rounded-xl bg-card border border-border shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-1 px-6 pt-4">
          <button
            onClick={() => setTab("info")}
            className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${tab === "info" ? "bg-primary text-primary-foreground" : "hover:bg-accent text-muted-foreground"}`}
          >
            Informações
          </button>
          <button
            onClick={() => setTab("edit")}
            className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${tab === "edit" ? "bg-primary text-primary-foreground" : "hover:bg-accent text-muted-foreground"}`}
          >
            Editar
          </button>
        </div>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t">
          {tab === "info" ? (
            <>
              <button onClick={onClose} className="rounded-lg border border-border/60 bg-muted/40 px-4 py-2 text-sm font-medium hover:bg-muted transition-colors">Fechar</button>
            ...
            <>
              <button onClick={() => setTab("info")} disabled={saving} className="rounded-lg border border-border/60 bg-muted/40 px-4 py-2 text-sm font-medium hover:bg-muted transition-colors disabled:opacity-50">Cancelar</button>
