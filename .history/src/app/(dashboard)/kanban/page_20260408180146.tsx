...
import { GripVertical, User, X, Eye, Edit, Phone, Mail, Building2, Star } from "lucide-react"
...
const QUALIFICATION_LABELS: Record<string, { label: string; color: string }> = {
  quente:           { label: "Quente",          color: "text-red-500 bg-red-500/10 border-red-500/30" },
  morno:            { label: "Morno",            color: "text-amber-500 bg-amber-500/10 border-amber-500/30" },
  frio:             { label: "Frio",             color: "text-blue-400 bg-blue-400/10 border-blue-400/30" },
  nao_qualificado:  { label: "Não qualificado",  color: "text-muted-foreground bg-muted border-border" },
}
...
function LeadAvatar({ lead, size = "md" }: { lead: KanbanLead; size?: "sm" | "md" | "lg" }) {
  ...
  if (lead.profilePicUrl) {
    return <img src={lead.profilePicUrl} alt={lead.name} className={`${cls} rounded-full object-cover flex-shrink-0 border border-border`} />
  }
  return (
    <div className={`${cls} rounded-full bg-primary/20 flex items-center justify-center font-bold text-primary flex-shrink-0`}>
            ...
            <div className="min-w-0">
              ...
              <div className="mt-1.5">
                <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium ${qual.color}`}>
                  <Star className="h-2.5 w-2.5" />{qual.label}
                </span>
              </div>
            </div>
          </div>
                      ...
                      <CardContent className="p-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-1">
                              ...
                              <button
                                ...
                                className="flex-shrink-0 rounded-md p-1 text-muted-foreground hover:text-foreground hover:bg-accent transition-colors opacity-0 group-hover:opacity-100"
                                title="Ver detalhes"
                              >
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap mt-2 ml-6">
                          <span className={`inline-flex items-center gap-0.5 rounded-full border px-1.5 py-0.5 text-[10px] font-medium ${qual.color}`}>
                            <Star className="h-2 w-2" />{qual.label}
                          </span>
                        ...
                        <div className="flex items-center justify-between mt-2 ml-6">
                          ...
                          {lead.assignedTo ? (
                            <div className="flex h-5 w-5 items-center justify-center rounded-full bg-secondary text-[8px] font-bold text-secondary-foreground" title={lead.assignedTo.name}>
                              {getInitials(lead.assignedTo.name)}
                            </div>
                          ) : (
                        ...
                        {lead.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2 ml-6">
                            ...
                            {lead.tags.length > 3 && (
                              <span className="rounded-md bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">+{lead.tags.length - 3}</span>
                            )}
                          </div>
                        )}
                      </CardContent>
