...
function LeadDetailModal({ lead, onClose, onUpdated }: { lead: KanbanLead; onClose: () => void; onUpdated?: () => void }) {
  ...
  const inputCls = "w-full rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
  const selectCls = "w-full rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
      ...
      <div className="w-full max-w-lg rounded-xl bg-card border border-border shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t">
          {tab === "info" ? (
            ...
            <>
              <Button variant="outline" onClick={() => setTab("info")} disabled={saving}>Cancelar</Button>
