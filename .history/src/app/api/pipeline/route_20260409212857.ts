...
export async function GET() {
  try {
    ...
    // Buscar realtimeSentiment da tabela conversations (lazy migration fields)
    const leadIds = leads.map((l) => l.id)
    let sentimentMap = new Map()

    if (leadIds.length > 0) {
      try {
        // Buscar conversas mais recentes com sentimento
        ...
        console.warn("[Pipeline] Erro ao buscar sentimentos (conversations):", e)
        // Fallback: tentar buscar da AiAnalysis
        try {
          ...
          for (const row of analyses || []) {
            ...
            let urgency = score < 0 ? Math.min(100, Math.abs(score)) : 0
            sentimentMap.set(row.lead_id, { sentiment, urgency })
          }
        } catch (e2) {
          console.warn("[Pipeline] Erro ao buscar sentimentos (ai_analyses):", e2)
        }
      }
    }
