...
async function processFile(knowledgeId: string, file: File) {
  try {
    ...
    const knowledge = await (defaultPrisma.agentKnowledge as any).findUnique({
    ...
    await (defaultPrisma.agentKnowledgeChunk as any).deleteMany({ where: { knowledgeId } })

    // 5. Salva os chunks com embeddings
    if (agentId && chunks.length > 0) {
      await (defaultPrisma.agentKnowledgeChunk as any).createMany({
    ...
    await (defaultPrisma.agentKnowledge as any).update({
      where: { id: knowledgeId },
      data: {
        content: text,
