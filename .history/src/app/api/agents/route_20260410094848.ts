...
export async function GET(req: NextRequest) {
  try {
    ...
    // Garante que colunas novas existem (migration lazy)
    await prisma.$executeRawUnsafe(`ALTER TABLE agents ADD COLUMN calendar_enabled INTEGER NOT NULL DEFAULT 0`).catch(() => {})
    await prisma.$executeRawUnsafe(`ALTER TABLE agents ADD COLUMN calendar_add_meet_link INTEGER NOT NULL DEFAULT 1`).catch(() => {})
    ...
    const extraFields = await prisma.$queryRaw<{
      ...
      SELECT id, avatar_url, voice_enabled, voice_mode, voice_id, voice_speed, voice_stability, voice_similarity, typing_delay, typing_delay_max, mark_as_read, split_messages, learning_policy, supervisor_phone, calendar_enabled, calendar_add_meet_link FROM agents
    `
    ...
    const enriched = agents.map((a: any) => {
      ...
      return {
        ...
        supervisorPhone: extra.supervisor_phone ?? null,
        calendarEnabled: Boolean(extra.calendar_enabled),
        calendarAddMeetLink: extra.calendar_add_meet_link !== null ? Boolean(extra.calendar_add_meet_link) : true,
      }
    })
