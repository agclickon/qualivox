...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const agents = await prisma.$queryRaw<{ id: string; name: string; mode: string; connection_ids: string }[]>`
      SELECT id, name, mode, connection_ids FROM agents WHERE is_active = 1
    `

    // 2. Mensagens respondidas por agente
    const actionRows = await prisma.$queryRawUnsafe<{ agent_id: string; cnt: string; escalated: string }[]>(`
      SELECT
    ...
    const sentimentRows = await prisma.$queryRaw<{ conversation_id: string; sentiment_score: number; extracted_data: string | null; connection_id: string | null }[]>`
      ...
      WHERE a.conversation_id IS NOT NULL
        ...
        AND a.rowid IN (
          SELECT MAX(rowid) FROM ai_analyses
          WHERE conversation_id IS NOT NULL AND created_at >= ${sinceIso}
    ...
    const trendRows = await prisma.$queryRawUnsafe<{ day: string; agent_id: string; cnt: string }[]>(`
      SELECT
