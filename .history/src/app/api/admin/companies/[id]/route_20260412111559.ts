...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Busca dados atuais para evitar conflito de unique no CNPJ
    const current = await prisma.saasCompany.findUnique({ where: { id } })
    if (!current) {
    ...
    // Só inclui CNPJ se foi alterado (evita conflito de unique)
    const cnpjClean = cnpj?.trim() || null
    if (cnpjClean !== current.cnpj) {
    ...
    console.log("[API Company PUT] updateData:", JSON.stringify(updateData))

    const company = await prisma.saasCompany.update({ where: { id }, data: updateData })
