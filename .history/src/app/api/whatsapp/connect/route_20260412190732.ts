...
async function connectConnection(connectionId: string) {
  ...
  // Sempre remove sessão presa antes de iniciar nova (creds inválidas ou OPENING travado)
  const existingSession = sessions.get(connection.id)
  const isStuck = !getSession(connection.id) && existingSession // está em sessions mas não em connectedIds
  if (isStuck) {
    console.log(`[Connect] Removendo sessão presa para ${connection.id}`)
