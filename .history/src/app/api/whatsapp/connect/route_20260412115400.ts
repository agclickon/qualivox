...
async function connectConnection(connectionId: string) {
  let connection: WaConn | null = await defaultPrisma.whatsappConnection.findUnique({ where: { id: connectionId } })
  ...
  const refreshed = await defaultPrisma.whatsappConnection.findUnique({ where: { id: connection.id } })
  if (refreshed) connection = refreshed

  if (connection!.status !== "CONNECTED" && !(connection!.status === "qrcode" && connection!.qrcode)) {
    for (let attempt = 0; attempt < 8; attempt++) {
      await new Promise((resolve) => setTimeout(resolve, 1000))
      const polled: WaConn | null = await defaultPrisma.whatsappConnection.findUnique({ where: { id: connection!.id } })
      if (!polled) break
      connection = polled
