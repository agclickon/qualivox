...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    // Busca conversa com lead, mensagens e atendente
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const conv = await (prisma.conversation.findUnique as any)({
    ...
    const leadName = conv.lead?.name || "Lead"

    // Determina nome do agente/atendente
    let agentName = "Atendente"
    if (conv.assignedTo?.name) {
      agentName = conv.assignedTo.name
    } else {
      // Tenta buscar agente ativo para a conexão
      try {
        const agentRow = await prisma.$queryRaw<{ name: string }[]>`
          SELECT name FROM agents WHERE is_active = 1 LIMIT 1
        `
        if (agentRow[0]?.name) agentName = agentRow[0].name
      } catch { /* ignora */ }
    }
    ...
    for (const msg of conv.messages) {
      ...
      if ((type === "audio" || type === "voice" || type === "ptt") && msg.mediaUrl) {
        hasAudio = true
        ...
        if (apiKey) {
          try {
            ...
            const filePath = resolveMediaPath(msg.mediaUrl)
        ...
        let meta: Record<string, unknown> = {}
        try { meta = msg.metadata ? JSON.parse(msg.metadata as string) : {} } catch { /* */ }
        const filename = (meta.fileName as string) || "arquivo"
    ...
    const startedAt = conv.messages[0] ? formatDateTime(conv.messages[0].createdAt) : "—"
    const endedAt = conv.messages.at(-1) ? formatDateTime(conv.messages.at(-1)!.createdAt) : "—"
