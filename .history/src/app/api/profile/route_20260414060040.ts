...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const currentUser = await defaultPrisma.user.findUnique({
    ...
    const emailAlreadyUsed = await defaultPrisma.user.findFirst({
      where: {
        email,
        NOT: { id: authUser.id },
      },
    ...
    if (avatarFile) {
      ...
      newAvatarUrl = await saveAvatarFile(authUser.id, buffer, extension)
    }
    ...
    console.error("[profile] Erro ao atualizar perfil:", errMsg, error)
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: `Erro interno: ${errMsg}` } },
      { status: 500 }
    )
  }
}
