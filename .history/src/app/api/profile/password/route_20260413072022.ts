...
export async function POST(request: NextRequest) {
  try {
    ...
    const user = await defaultPrisma.user.findUnique({ where: { id: authUser.id }, select: { passwordHash: true } })
    ...
    await defaultPrisma.user.update({ where: { id: authUser.id }, data: { passwordHash: newHash } })
