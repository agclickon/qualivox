...
// Banco tenant com dados reais da Clickon
const SQLITE_PATH = path.join(__dirname, "..", "data", "tenants", "qualivox-2f1a8058-25e8-48cc-9dcd-a20ac3b69456.db")
...
async function main() {
  ...
  for (const table of TABLES) {
    try {
      ...
      // Get column names and PostgreSQL types — only use columns that exist in both
      const pgTypes = await getColumnTypes(table.name)
      const pgColumnSet = new Set(Object.keys(pgTypes))
      const columns = Object.keys(rows[0]).filter(col => pgColumnSet.has(col))
