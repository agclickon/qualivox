    ...
    if (!convInfo.assigned_name) {
      try {
        ...
        const agentRows = await prisma.$queryRawUnsafe<{ name: string }[]>(
          connId
            ? `SELECT a.name FROM agents a WHERE a.is_active = true AND (a.connection_ids LIKE $1 OR a.connection_ids = '[]') LIMIT 1`
      ...
      if ((type === "audio" || type === "voice" || type === "ptt") && msg.media_url) {
        ...
        if (apiKey) {
          try {
            ...
            const buffer = await getMediaFileBuffer(msg.media_url!)
