...
import { onMessageStatus, onNewMessage, onConversationUpdate } from "@/lib/baileys-listener"
    ...
    start(controller) {
      ...
      const unsubStatus = onMessageStatus((_event, data) => {
        if (data && typeof data === "object") {
          sendEvent({ type: "message_status", ...(data as Record<string, unknown>) })
        } else {
          sendEvent({ type: "message_status", payload: data })
        }
      })
      ...
      // Subscribe to conversation updates (autostop, etc.)
      const unsubConvUpdate = onConversationUpdate((_event, data) => {
        sendEvent({ type: "conversation_update", ...(data as Record<string, unknown>) })
      })
