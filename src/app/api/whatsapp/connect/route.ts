...
async function connectConnection(connectionId: string) {
  let connection: WaConn | null = await prisma.whatsappConnection.findUnique({ where: { id: connectionId } })
  ...
  const refreshed = await prisma.whatsappConnection.findUnique({ where: { id: connection.id } })
  ...
  if (connection!.status !== "CONNECTED" && !(connection!.status === "qrcode" && connection!.qrcode)) {
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const polled: WaConn | null = await prisma.whatsappConnection.findUnique({ where: { id: connection!.id } })
      if (!polled) break
      connection = polled
